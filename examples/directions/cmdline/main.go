...
func getDirections(ctx context.Context, origin, destination string) {
    d, err := directions.Directions(ctx, origin, destination)
    if err != nil {
        log.Fatalf("Directions failed, %v", err)
    }
    fmt.Println(d)
}
...
func main() {
    ...
    resp, err := directions.Directions(ctx, *origin, *destination)
    if err != nil {
